...
### 버그 수정
- **데이터셋 정밀도 — 얼굴 정체성 일관성 검사 추가(`identity.py`).** 기존 파이프라인은 각
  이미지를 개별 평가(품질+VL)만 해서, 개별적으로는 선명·구도 양호하나 **다른 사람** 얼굴인
  컷이 그대로 통과해 identity LoRA 데이터셋을 오염시켰다. OpenCV SFace(YuNet 검출기의 인식
  짝, ~37MB ONNX·CPU·추가 pip 의존성 없음)로 얼굴 임베딩을 계산하고, 데이터셋의 **다수
  정체성**과 코사인 유사도(기본 0.363)가 낮은 컷을 `off-identity` 하드 리젝트로 처리한다.
  단일 인물 목적(`face`, `full_body`)에만 적용되며, 얼굴 수가 적거나 다수 정체성이 불분명하면
  오탐 방지를 위해 안전하게 건너뛴다. 임계값/활성화는 `IdentityConfig`로 per-job 오버라이드 가능.
- **유저 ID 노출.** 추적 파일(`.env.example`, `docker-compose.yml`, `docs/docker.md`)의 예시
  경로에 실제 호스트 사용자명(`interbus`)이 하드코딩되어 있던 것을 `youruser` 플레이스홀더로 교체.
